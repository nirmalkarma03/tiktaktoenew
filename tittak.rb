...
class Tiktak
  ...
  def game_draw
	...
	def digonal_prediction
	  ...
	  if row1.uniq.count==2 && row1.include?('_')
	  	return [row1.index('_'),row1.index('_')]
	  elsif row2.uniq.count==2 && row2.include?('_')
	  	temp1=row2.index('_')
	...
	def for_computer_win
		row, column, digonal = row_prediction, column_prediction, digonal_prediction
		# row,colunm,digonal=cwin_row_prediction,cwin_column_prediction,cwin_diagonal_prediction
		if row&.count == 2
			row
		elsif column&.count == 2
			column
	...
	# 	@screen.each_with_index{ |row,inx| return [inx,row.index('_')]   if row.uniq.count==2&&row.include?('_')}
	# end

	# def cwin_column_prediction
	# 	(0..2).each do |col|
	# 		row=[@screen[0][col],@screen[1][col],@screen[2][col]]
	# 		return [row.index('_'),col]  if row.uniq.count==2&&row.include?('_')
	# end
